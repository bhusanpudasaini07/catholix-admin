...
const useInactiveDevices = () => {
  ...
  /**
   * Inactive Devices Stats
   */
  const { data: inactiveDevicesStats, isLoading: inactiveDevicesStatsLoading } =
    useQuery<IInactiveStats>({
  ...
  useEffect(() => {
    const localStorageColumns = localStorage.getItem(
      "columnVisibility_inactive_devices"
    );
    if (localStorageColumns) {
      ...
      const visibleColumns = inactiveDeviceColumns?.reduce(
