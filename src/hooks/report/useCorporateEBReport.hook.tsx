...
const useCorporateEBReport = () => {
  ...
  const { data: corporateEBReport, isLoading: corporateEBReportLoading } =
    useQuery<IProps>({
      queryKey: ["corporate-eb-report", searchTrigger],
      queryFn: () =>
        getCorporateEBReport(
  ...
  const transformData = (data: IDashboardReport | undefined) => {
    ...
    const columns: ColumnDef<any>[] = [
      ...
      ...dates.map((date) => ({
        ...
        id: date,
        cell: ({ row }: any) => (
          <Badge variant={"info"}>{row.getValue(date)}</Badge>
        ),
      })),
    ];

    const categories = (Object.keys(data) as (keyof IDashboardReport)[]).map(
      (key) => {
  ...
  const transformedData = transformData(corporateEBReport?.data);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      const fileName = `corporate_eb_report_${moment(new Date()).format(
        "YYYY-MM-DD"
      )}.csv`;
...
export default useCorporateEBReport;
