...
import {
  IConversionRateDealer,
  IConversionRateDealerHeader,
  IConversionRateDealerResponse,
...
interface IProps {
  data: IConversionRateDealer;
}

const useConversionRateDealer = () => {
  ...
  const { data: conversionRateData, isLoading: conversionRateLoading } =
    useQuery<IProps>({
      queryKey: ["dealerConversionRate", searchTrigger, page, perPage],
      queryFn: async () => {
        if (regionId && stateId) {
          return await getDealerConversionRate(
            page,
            perPage,
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      const fileName = `dealer_conversion_rate_${moment(new Date()).format(
        "YYYY-MM-DD"
      )}.csv`;
  ...
  //   Columns
  const generateColumns = (): ColumnDef<IConversionRateDealerResponse>[] => {
    const columns: ColumnDef<IConversionRateDealerResponse>[] = [
      {
        id: "sn",
        header: "S.N",
        cell: ({ row }) => (
    ...
    if (conversionRateData?.data?.headers) {
      conversionRateData?.data?.headers?.forEach(
        (header: IConversionRateDealerHeader) => {
          columns.push({
...
export default useConversionRateDealer;
