...
import {
  IConversionRateAgent,
  IConversionRateAgentHeader,
  IConversionRateAgentResponse,
...
interface IProps {
  data: IConversionRateAgent;
}

const useCoversionRateAgent = () => {
  ...
  const { data: conversionRateData, isLoading: conversionRateLoading } =
    useQuery<IProps>({
      queryKey: ["conversionRate", searchTrigger, page, perPage],
      queryFn: async () => {
        if (regionId && stateId) {
          return await getAgentConversionRate(
            page,
            perPage,
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      const fileName = `agent_conversion_rate_${moment(new Date()).format(
        "YYYY-MM-DD"
      )}.csv`;
  ...
  //   Columns
  const generateColumns = (): ColumnDef<IConversionRateAgentResponse>[] => {
    const columns: ColumnDef<IConversionRateAgentResponse>[] = [
      {
        id: "sn",
        header: "S.N",
        cell: ({ row }) => (
    ...
    if (conversionRateData?.data?.headers) {
      conversionRateData?.data?.headers?.forEach(
        (header: IConversionRateAgentHeader) => {
          columns.push({
...
export default useCoversionRateAgent;
