...
const ImagePdfDsiplay = ({ invoice_file }: IProps) => {
  const zoomPluginInstance = zoomPlugin();
  const { Zoom } = zoomPluginInstance;
  return invoice_file && invoice_file?.match(/\.(jpeg|jpg|png)$/) ? (
    <div className="py-6 px-4 h-[calc(100vh-210px)] overflow-y-scroll">
      <Magnifier src={invoice_file} width={500} mgShape="circle" />
    </div>
  ) : (
...
export default ImagePdfDsiplay;
