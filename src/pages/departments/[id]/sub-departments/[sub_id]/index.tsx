...
const SubDepartmentDetail: NextPageWithLayout = () => {
  ...
  const { data: departmentDetail, isLoading } =
    useQuery<IDepartmentDetailProps>({
      queryFn: async () => {
        if (sub_id) {
          const response = await getDepartmentDetail(sub_id);
          return response;
        }
      },
      queryKey: ["departmentDetail", sub_id],
    });
  return (
    <div className="flex flex-col max-w-4xl gap-6 m-auto">
      <div className="flex items-center gap-6">
        <Link
          href={`/departments/${id}/sub-departments`}
...
SubDepartmentDetail.getLayout = (page) => {
