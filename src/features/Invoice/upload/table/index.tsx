...
interface IExtractDetailProps {
  extractDetail: any;
}

const UploadInvoiceTable = ({ extractDetail }: IExtractDetailProps) => {
  /**
    ...
    <div className="border">
      <Table>
        <TableHeader className="bg-purple-60 text-white">
          <TableRow>
            {extractDetail?.items &&
              extractDetail?.items[0] &&
              generateOrderedKeys(extractDetail?.items[0]).map((key, index) => (
                <TableHead
                  key={index}
        ...
        <TableBody>
          {/* {dummyData.map((data) => ( */}
          {extractDetail?.items?.map((item: any, index: number) => (
            <TableRow key={index}>
